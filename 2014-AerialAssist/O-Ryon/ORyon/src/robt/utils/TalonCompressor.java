 ...
 * compressor.
 */
public class TalonCompressor extends SensorBase implements IDevice, LiveWindowSendable{

    private DigitalInput m_pressureSwitch;
    private Talon m_speedcontroller;
    ...
    private class CompressorThread extends Thread {
        ...
        TalonCompressor m_compressor;

        CompressorThread(TalonCompressor comp) {
            m_compressor = comp;
        }
        ...
        public void run() {
            while (m_run) {
                if (m_compressor.enabled()) {
                    m_compressor.setPower(!m_compressor.getPressureSwitchValue() ? 1 : 0);
                } else {
                    m_compressor.setPower(0);
                }
                try {
                    Thread.sleep(500);
    ...
    private void initCompressor(final int pressureSwitchSlot,
        ...
        m_speedcontroller = new Talon(compresssorRelaySlot, compressorRelayChannel);
     ...
     */
    public TalonCompressor(final int pressureSwitchSlot,
            final int pressureSwitchChannel,
            final int compresssorRelaySlot,
            final int compressorRelayChannel) {
     ...
     * @param compressorRelayChannel The relay channel that the compressor relay is attached to.
     */
    public TalonCompressor(final int pressureSwitchChannel, final int compressorRelayChannel) {
