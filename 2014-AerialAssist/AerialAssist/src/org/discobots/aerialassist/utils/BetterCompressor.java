...
public class BetterCompressor extends SensorBase implements IDevice, LiveWindowSendable{
    ...
    private class CompressorThread extends Thread {
        ...
        BetterCompressor m_compressor;

        CompressorThread(BetterCompressor comp) {
            m_compressor = comp;
        }
    ...
    private void initCompressor(final int pressureSwitchSlot,
        ...
        m_victor = new Victor(victorRelaySlot, victorRelayChannel);
     ...
     */
    public BetterCompressor(final int pressureSwitchSlot,
            final int pressureSwitchChannel,
            final int compresssorRelaySlot,
            final int compressorRelayChannel,
            final int compressorVictorSlot,
            final int compressorVictorChannel) {
        initCompressor(pressureSwitchSlot,
                pressureSwitchChannel,
                compresssorRelaySlot,
                compressorRelayChannel,
                compressorVictorSlot,
                compressorVictorChannel);
    }
    ...
    public void setRelayValue(Relay.Value relayValue) {
        ...
        if (relayValue == Relay.Value.kOn || relayValue == Relay.Value.kForward) {
            m_victor.set(1);
        } else if (relayValue == Relay.Value.kOff) {
            m_victor.set(0);
        } else if (relayValue == Relay.Value.kReverse) {
            m_victor.set(-1);
        }
    }
