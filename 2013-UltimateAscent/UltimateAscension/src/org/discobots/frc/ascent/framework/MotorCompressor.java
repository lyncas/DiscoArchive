...
/**
 * Modified by Sam to use speed controller
 */
 ...
 * compressor.
 */
public class MotorCompressor extends SensorBase implements IDevice, LiveWindowSendable{

    private DigitalInput m_pressureSwitch;
    private SpeedController m_motorController;
    ...
    private class CompressorThread extends Thread {
        ...
        MotorCompressor m_compressor;

        CompressorThread(MotorCompressor comp) {
            m_compressor = comp;
        }
        ...
        public void run() {
            while (m_run) {
                if (m_compressor.enabled()) {
                    m_compressor.setValue(!m_compressor.getPressureSwitchValue() ? true : false);
                } else {
                    m_compressor.setValue(false);
                }
                try {
                    Thread.sleep(500);
    ...
    private void initCompressor(final int pressureSwitchSlot,
        ...
        m_motorController = sc;

        UsageReporting.report(UsageReporting.kResourceType_Compressor, 0);

        m_task = new MotorCompressor.CompressorThread(this);
        m_task.start();
    }
     ...
     */
    public MotorCompressor(final int pressureSwitchSlot,
            final int pressureSwitchChannel,
            SpeedController sc) {
     ...
     * @param compressorRelayChannel The relay channel that the compressor relay is attached to.
     */
    public MotorCompressor(final int pressureSwitchChannel, SpeedController sc) {
    ...
    public void free() {
        ...
        //m_motorController.free();
        m_pressureSwitch = null;
        m_motorController = null;
    }
