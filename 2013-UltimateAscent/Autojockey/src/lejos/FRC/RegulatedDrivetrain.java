...
/**
 * FRC drivetrain side (left or right) abstraction. speeds and degrees/sec are
 * measured at motor output end.
 *
...
//        reg = new Regulator();
//    }
    public RegulatedDrivetrain(Victor motor1, Victor motor2, boolean motor1Reversed, boolean motor2Reversed, Encoder enc, double enc_ticks_per_wheel_rev) {
        this.motor1 = motor1;
        this.motor1_multiplier = motor1Reversed ? -1 : 1;
        this.motor2 = motor2;
        this.motor2_multiplier = motor2Reversed ? -1 : 1;
        this.enc = enc;
        this.ticksPerRev = enc_ticks_per_wheel_rev;
        enc.start();
        reg = new Regulator();
    }

    public RegulatedDrivetrain(Victor motor1, Victor motor2, Encoder enc, double enc_ticks_per_wheel_rev) {
        this(motor1, motor2, false, false, enc, enc_ticks_per_wheel_rev);
    }
    public RegulatedDrivetrain(Victor motor1, Encoder enc, double enc_ticks_per_wheel_rev) {
        this(motor1, null, false, false, enc, enc_ticks_per_wheel_rev);
    }

    /**
     * Removes this motor from the motor regulation system. After this call the
     * motor will be in float mode and will have stopped. Note calling any of
     * the high level move operations (forward, rotate etc.), will automatically
     * enable regulation.
     *
    ...
    /**
     * @return the current tachometer count. Returns the degrees the wheels have
     * turned
     * @see lejos.robotics.RegulatedMotor#getTachoCount()
     */
    public int getTachoCount() {
        return (int) (enc.get() / ticksPerRev * 360);
    }
     ...
     * circumstances (activeMotors that are in the process of stalling, or
     * activeMotors that have been forced out of position), the two values may
     * differ. Note this value is not valid if regulation has been terminated.
     *
     ...
     * motor doesn't just stop; it will resist any further motion. Cancels any
     ...
     * For example, If the motor is stalled, isMoving() will return <b> true.
     * </b><br>
     * After flt() is called, this method will return <b>false</b> even though
     * the motor axle may continue to rotate by inertia. If the motor is
     * stalled, isMoving() will return <b> true. </b> . A stall can be detected
     * by calling {@link #isStalled()};
     *
     ...
     * The default value is 800; Smaller values will make speeding up or
     * stopping at the end of a rotate() task smoother;
     *
     * @param acceleration
     */
     ...
     * will cause any current move operation to be halted. PLEASE DON'T CALL
     * THIS. It's not really synchronized with the drivetrain.
     */
    ...
    /**
     * Set the parameters for detecting a stalled motor. A motor will be
     * recognised as stalled if the movement error (the amount the motor lags
     * the regulated position) is greater than error for a period longer than
     * time.
     *
    ...
    /**
     * Inner class to regulate velocity; also stop motor at desired rotation
     * angle. This class uses a very simple movement model based on simple
     * linear acceleration. This model is used to generate ideal target
     * positions which are then used to generate error terms between the actual
     * and target position this error term is then used to drive a PID style
     * motor controller to regulate the power supplied to the motor.
     *
     * If new command are issued while a move is in progress, the new command is
     * blended with the current one to provide smooth movement.
     *
     ...
     *
     * Once the motor stops, the final position is held using the same PID
     * control mechanism (with slightly different parameters), as that used for
     * movement.
     *
     */
...
//        float MOVE_D = 22f;
        ...
        /**
         * Helper method. Start a sub move operation. A sub move consists of
         * acceleration/deceleration to a set velocity and then holding that
         * velocity up to an optional limit point. If a limit point is set this
         * method will be called again to initiate a controlled deceleration to
         * that point
         *
         * @param speed
         * @param acceleration
        ...
        /**
         * Initiate a new move and optionally wait for it to complete. If some
         * other move is currently executing then ensure that this move is
         * terminated correctly and then start the new move operation.
         *
         * @param speed
         * @param acceleration
         * @param limit
        ...
        /**
         * Monitors time and tachoCount to regulate velocity and stop motor
         * rotation at limit angle
         */
        ...
        /**
         * helper method for velocity regulation. calculates power from error
         * using double smoothing and PID like control
         *
         * @param error
         */
    ...
    /**
     * This class provides a single thread that drives all of the motor
     * regulation process. Only active motors will be regulated. To try and keep
     * motors as closely synchronized as possible. tach counts for all motors
     * are gathered as close as possible to the same time. Similarly new power
     * levels for each motor are also set at the same time.
     */
