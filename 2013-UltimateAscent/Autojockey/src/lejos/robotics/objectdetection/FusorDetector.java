 ...
 */
/**
 * <p>If you have a robot with multiple sensors (touch and range) and would like
 * them to report to one listener, or if you want to control them at the same
 * time (such as disabling them all at once) you can use this class.</p>
 *
...
public class FusorDetector implements FeatureDetector, FeatureListener {
    ...
    /**
     * The delay (in ms) between notifying listeners of detected features (if
     * any are detected).
     */
    private int delay;
    ...
    /**
     * This method adds another FeatureDetector to the FusorDetector. This
     * method will set the delay for this class to the largest delay if comes
     * across from all the FeatureDetector objects added to it.
     *
     * @param detector
     */
    ...
    /**
     * This method scans all the sensors added to this object and returns the
     * amalgamated results. NOTE: This method is not called by the thread code.
     */
    public Feature scan() {
    ...
    /**
     * This method must deal with different delays from different sensors. e.g.
     * Touch is 50 ms, while ultrasonic is 250 ms. But must amalgamate readings
     * that are captured over same period. SOLUTION: Use listeners. Get max scan
     * time (e.g. 250 ms) and wait that long for each listener to report in
     * before reporting all at once.
     */
	...
	for (Iterator it = detectors.iterator(); it.hasNext();) {
	    ...
	    if (fd instanceof RangeFeatureDetector) {
		((RangeFeatureDetector) fd).setPoseProvider(pp);
	    }
	}
    }
    ...
    /**
     * This method enables/disables automatic scanning and listener reporting
     * for this object and all FeatureDetectors used in this FusorDetector
     * object.
     */
